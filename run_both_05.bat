@echo off
REM ===========================================================================
REM run_both_05.bat  -  the corrected-physics pair.
REM Identical to run_both.bat except that it writes run05 and run06 instead of
REM run03 and run04, so the published catalogues are left untouched for the
...
REM
REM   run05  --mpi-trend 0.0   stationary control, the validation catalogue
REM   run06  --mpi-trend 4.0   warming experiment
REM
...
REM common ground, so the run05 against run06 comparison carries less sampling
...
REM random draws and the streams desynchronise. Read run05 against run06 as a
...
call :archive "%OUT%\run05" || goto :fail
call :archive "%OUT%\run06" || goto :fail

echo Cores detected: %NUMBER_OF_PROCESSORS%   threads per run: %THREADS%
echo.

start "SynTC run05 stationary" powershell -NoLogo -NoExit -Command "$env:OMP_NUM_THREADS='%THREADS%'; $env:MKL_NUM_THREADS='%THREADS%'; Set-Location '%~dp0'; python -u syntc_ai.py --years 2026 2125 --ensembles 20 --ibtracs '%IBTRACS%' --dtm '%DTM%' --out '%OUT%\run05' --mpi-trend 0.0 2>&1 | ForEach-Object { [string]$_ } | Tee-Object -FilePath '%OUT%\run05.log'"

start "SynTC run06 warming" powershell -NoLogo -NoExit -Command "$env:OMP_NUM_THREADS='%THREADS%'; $env:MKL_NUM_THREADS='%THREADS%'; Set-Location '%~dp0'; python -u syntc_ai.py --years 2026 2125 --ensembles 20 --ibtracs '%IBTRACS%' --dtm '%DTM%' --out '%OUT%\run06' --mpi-trend 4.0 2>&1 | ForEach-Object { [string]$_ } | Tee-Object -FilePath '%OUT%\run06.log'"
