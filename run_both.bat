...
REM
REM   run03  --mpi-trend 0.0   stationary control, the validation catalogue
REM   run04  --mpi-trend 4.0   warming experiment
REM
...
REM common ground, so the run03 against run04 comparison carries less sampling
...
REM random draws and the streams desynchronise. Read run03 against run04 as a
...
call :archive "%OUT%\run03" || goto :fail
call :archive "%OUT%\run04" || goto :fail

echo Cores detected: %NUMBER_OF_PROCESSORS%   threads per run: %THREADS%
echo.

start "SynTC run03 stationary" powershell -NoLogo -NoExit -Command "$env:OMP_NUM_THREADS='%THREADS%'; $env:MKL_NUM_THREADS='%THREADS%'; Set-Location '%~dp0'; python -u syntc_ai.py --years 2026 2125 --ensembles 20 --ibtracs '%IBTRACS%' --dtm '%DTM%' --out '%OUT%\run03' --mpi-trend 0.0 2>&1 | ForEach-Object { [string]$_ } | Tee-Object -FilePath '%OUT%\run03.log'"

start "SynTC run04 warming" powershell -NoLogo -NoExit -Command "$env:OMP_NUM_THREADS='%THREADS%'; $env:MKL_NUM_THREADS='%THREADS%'; Set-Location '%~dp0'; python -u syntc_ai.py --years 2026 2125 --ensembles 20 --ibtracs '%IBTRACS%' --dtm '%DTM%' --out '%OUT%\run04' --mpi-trend 4.0 2>&1 | ForEach-Object { [string]$_ } | Tee-Object -FilePath '%OUT%\run04.log'"
