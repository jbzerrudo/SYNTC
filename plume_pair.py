...
CENTRAL_COLOR = "#08306b"   # the middle track, blue
...
_ap.add_argument("--min-steps", type=int, default=16,
                 help="shortest realisation eligible to be drawn, in 6-hourly "
                      ...
                      "into the sample.")
_ap.add_argument("--central", default="off",
...
def draw_central(ax, x, y, colour=CENTRAL_COLOR, lw=0.75, z=5.5):
    if x is None or len(x) < 2:
        return
    # A 0.75 line in blue disappears over the dark cells without a thin
    # white stroke behind it. Drop the path_effects to remove it.
...
def panel(ax, tag, glat, glon, label, nkeep=None, note=""):
    ...
    sc = pd.Series(score)
    keep = choose(sc, tr, NKEEP if nkeep is None else nkeep)
    for sid in keep:
...
print(f"drawn: {nL} of {tL} eligible (a), {nR} of {tR} (b), selection={A.pick}")
