...
FORECAST_PATH = os.path.join(DATA_DIR, "NF100Y_v3.csv")
...
# Load DEM immediately at startup so all land checks work correctly
try:
    initialize_dem(DEM_PATH, DEM_FALLBACK_PATH)
except Exception as e:
    logging.error(f"Startup DEM loading failed: {e}")

# Define a larger boundary for genesis points
GENESIS_BOUNDS = [
    (25.00, 141.461026),  # 25.0°N, 141.461026°E
    (5.00, 141.461026),   # 5.0°N, 141.461026°E
    (7.00, 110.00),   # 5.0°N, 114.115689°E
    (25.00, 110.00),  # 25.0°N, 114.115689°E
]
...
# ——— Genesis Exclusion Zone ———
# No TC genesis should occur within the Philippine archipelago interior
# (inter-island seas: Sulu Sea, Sibuyan Sea, Visayan Sea, Samar Sea, etc.)
# This covers the region between Palawan and the eastern Visayas/Samar coast.
# A polygon is used instead of a simple lon/lat box to allow SCS genesis
# west of Palawan and open-ocean genesis east of Samar at higher latitudes.
_GENESIS_EXCLUSION_RAW = Polygon([
    (117.0, 5.0),    # SW corner — south of Palawan
    (126.0, 5.0),    # SE corner — south of Mindanao  
    (126.0, 10.0),   # East of Mindanao / S. Leyte
    (126.5, 12.0),   # East of Samar — allow narrow passage
    (126.0, 14.0),   # East of S. Luzon (Bicol)
    (122.0, 14.5),   # South of Luzon — close off Sibuyan Sea
    (119.5, 14.5),   # West of Luzon — close off Mindoro Strait
    (117.0, 14.5),   # West of Palawan — extended north to close Mindoro gap
    (117.0, 5.0),    # Back to SW corner
])
GENESIS_EXCLUSION_POLYGON = _GENESIS_EXCLUSION_RAW.buffer(0.15)  # ~17km buffer for edge cases

# ——— Sulu Sea Weak-TD Sub-Zone (CONSERVATIVE) ———
# Centered on the IHO Sulu Sea deep central basin (centroid ~9.0°N, 120.5°E).
# IBTrACS 1923–2023: ~0.39 storms/year, 28.9% of years have ≥1, almost all TD/RL.
# Boundaries keep >0.5° from nearest major coastline:
#   North 9.5°N: >1° south of Panay (Nasog Pt 11.9°N)
#   South 7.0°N: above Sulu Archipelago shallows
#   West 119.5°E: east of Palawan shelf
#   East 121.5°E: west of Negros/Zamboanga coast
SULU_SEA_GENESIS_ZONE = Polygon([
    (119.5,  7.0),
    (121.5,  7.0),
    (121.5,  9.5),
    (119.5,  9.5),
...
# ——— Palawan–Mindoro Gap Hard-Exclusion ———
# Blocks genesis of ANY intensity in the strait between Mindoro and Palawan,
# and the Visayan Sea. Applied BEFORE the SCS-fringe check to plug any leaks.
# Starts at 119.0°E so it doesn't block legitimate open-SCS genesis west of Palawan.
PALAWAN_MINDORO_GAP = Polygon([
    (119.0, 10.0),
    (122.0, 10.0),
    (122.0, 14.5),
    (119.0, 14.5),
...
def is_in_sulu_sea_genesis_zone(lon, lat):
    """Check if point is in the tight central Sulu Sea open-water basin."""
    return SULU_SEA_GENESIS_ZONE.contains(Point(lon, lat))

def is_in_palawan_mindoro_gap(lon, lat):
    """Check if point is in the Palawan–Mindoro strait (hard exclusion)."""
    return PALAWAN_MINDORO_GAP.contains(Point(lon, lat))
    
# ——— Luzon Mainland Exclusion (DEM-independent) ———
# The main exclusion polygon stops at 14.5°N, but Luzon extends to ~19°N.
# This polygon traces the island outline so genesis is blocked over Luzon
# regardless of whether the GEBCO DEM loads. The Philippine Sea east of
# the Sierra Madre (~123°E+) remains open for legitimate open-ocean genesis.
LUZON_MAINLAND_EXCLUSION_RAW = Polygon([
    (119.3, 14.5),   # SW — connects to main exclusion at Mindoro Strait
    (119.5, 16.5),   # W — Zambales / Pangasinan coast
    (119.7, 18.0),   # NW — Ilocos Sur
    (120.0, 18.9),   # N — Pagudpud / Ilocos Norte (was 120.2 — too narrow)
    (121.5, 19.0),   # N — Aparri coast
    (122.7, 18.5),   # NE — Cape Engaño (was 122.5 — too narrow)
    (123.3, 17.0),   # E — Sierra Madre N (was 122.8 — missed east face)
    (123.5, 15.5),   # E — Aurora / Quezon coast (was 122.5 — missed Sierra Madre)
    (123.5, 14.0),   # SE — Bicol / Catanduanes
    (122.0, 14.5),   # S — connects to main exclusion
    (119.3, 14.5),   # Close polygon
])
LUZON_MAINLAND_EXCLUSION = LUZON_MAINLAND_EXCLUSION_RAW.buffer(0.15)

# ——— Historical SCS / Sulu Sea Genesis Rates (IBTrACS 1923–2023) ———
_HIST_SCS_MEAN_PER_YEAR = 4.27
_HIST_SULU_MEAN_PER_YEAR = 0.39
_HIST_SULU_NONZERO_FRAC = 0.289

# FIX-D: Minimum geographic bounds for SCS genesis near Borneo.
...
def is_in_genesis_exclusion_zone(lon, lat):
    """
    Check if a point falls within the Philippine archipelago interior
    OR over the Luzon mainland where TC genesis is physically unrealistic.
    """
    pt = Point(lon, lat)
    return (GENESIS_EXCLUSION_POLYGON.contains(pt) or
            LUZON_MAINLAND_EXCLUSION.contains(pt))
...
# ——— SCS GENESIS WIND CAPS (IBTrACS 1977–2023 calibrated) ———
# Maximum wind speed allowed for each assigned category when genesis is in SCS.
# These are the UPPER BOUNDS of each PAGASA category.
_SCS_CATEGORY_WIND_CAPS = {
    'TD':  33.0,   # Tropical Depression: 22–33 kt
    'TS':  47.0,   # Tropical Storm: 34–47 kt
    'STS': 63.0,   # Severe Tropical Storm: 48–63 kt
    'TY':  95.0,   # Typhoon: 64–99 kt (capped at 95 for SCS)
    'STY': 95.0,   # Should never occur for SCS genesis, but safety net
}
...
def sample_category_peak_wind(storm_category):
    """
    Sample a lifetime-maximum wind speed for a given storm category using
    beta-distribution parameters calibrated to the manuscript's Weibull
    return-level table (Table 3) and historical beta fits (Fig. 3).

    Returns:
        float: sampled peak wind speed (knots)
    """
    _BETA_PARAMS = {
        'TD':  (0.95, 1.20,  22.0,  33.0),
        'TS':  (0.97, 1.20,  34.0,  47.0),
        'STS': (0.34, 0.43,  48.0,  63.0),
        'TY':  (1.04, 1.42,  64.0,  99.0),
        # STY: beta(0.55, 0.95) on [106, 126] reproduces Weibull RP table:
        #   P(>=120|STY) ~ 19%, P(>=124|STY) ~ 6%, P(>=125|STY) ~ 3%
        'STY': (0.55, 0.95, 106.0, 126.0),
    }
    a, b, lo, hi = _BETA_PARAMS.get(storm_category, _BETA_PARAMS['TD'])
...
def scale_winds_to_target_mean(storms_df, target_mean=58.0):
    ...
    # Apply scaling BUT preserve STY winds (>=100 kt)
    extreme_mask = scaled_df['WIND'] >= 100.0
    extreme_winds = scaled_df.loc[extreme_mask, 'WIND'].copy()

    # Scale all winds
    scaled_df['WIND'] = scaled_df['WIND'] * scale_factor

    # Restore extreme winds to preserve STY storm profiles
    if extreme_mask.any():
        scaled_df.loc[extreme_mask, 'WIND'] = extreme_winds
        logging.info(f"Preserved {extreme_mask.sum()} extreme winds >=100kt from scaling")
...
def weibull_gev_blend(size, historical_data=None, month=None, region=None, year=None):
    ...
    if month in _monthly_category_dist:
        ...
        # Use manuscript Table 2 trend-adjusted distributions directly
        adjusted_dist = [
            dist[0] * 1.00,  # TD'  — already trend-adjusted
            dist[1] * 1.00,  # TS'  — already trend-adjusted
            dist[2] * 1.00,  # STS' — already trend-adjusted
            dist[3] * 1.00,  # TY'  — already trend-adjusted
            dist[4] * 1.00   # STY' — already trend-adjusted
        ]
        # Normalize to 100%
    ...
    # -- post-hoc auto-calibration to 58-knot target mean --
    # Protect >=100 kt winds from compression
    mean0 = blend.mean()
    if mean0 > 0:
        sty_mask = blend >= 100.0
        sty_saved = blend[sty_mask].copy()
        scale_to_target = 58.0 / mean0
        blend *= scale_to_target
        if sty_mask.any():
            blend[sty_mask] = sty_saved
        blend = np.clip(blend, 22.0, absolute_cap)
    ...
    # 2) Post-validate scale to 58 kt, preserving >=100 kt extreme tail
    mean_valid = validated.mean()
    if mean_valid > 0:
        sty_mask_v = validated >= 100.0
        sty_saved_v = validated[sty_mask_v].copy()
        scale_to_target = 58.0 / mean_valid
        validated = validated * scale_to_target
        if sty_mask_v.any():
            validated[sty_mask_v] = sty_saved_v
...
def adjust_wind_profile(base_winds, target_max_wind, storm_category):
    ...
    if storm_category == 'STY':
        # Super Typhoons: rapid intensification, brief sustained peak, long weakening
        # Historical: peak ~12-24h out of 7-10 day lifetime ≈ 10-15% of track
        ramp_up_fraction = np.random.uniform(0.25, 0.35)  # First 25-35% is intensification
        peak_fraction = np.random.uniform(0.12, 0.20)     # Next 12-20% is peak intensity
    ...
    for i in range(ramp_up_end, peak_end):
        ...
        # FIX: STY peak must not oscillate below 106 kt — historical STY
        # sustains ≥106 continuously over open water for many hours
        if storm_category == 'STY' and target_max_wind >= 106:
            adjusted_winds[i] = max(106.0, adjusted_winds[i])
        # FIX: TY peak must not exceed 99 kt (±3% fluctuation on 99 → 101.97)
        if storm_category == 'TY':
            adjusted_winds[i] = min(99.0, adjusted_winds[i])
    ...
    for i in range(peak_end, n):
        ...
        if storm_category == 'STY':
            # Super Typhoons: shortened linger zone (was 30% → now 15%),
            # steeper initial slope (was 0.5 → 0.8) so storm exits 120+ range faster
            if progress < 0.15:
                decay_factor = progress * 0.8   # Moderate initial decay
            else:
                decay_factor = 0.12 + (progress - 0.15) * 1.035  # Faster later decay
        elif storm_category == 'TY':
    ...
    # Enforce category ceiling (catches any post-smoothing or peak-region scaling leaks)
    _profile_caps = {'TD': 33.0, 'TS': 47.0, 'STS': 63.0, 'TY': 99.0, 'STY': 126.0}
    if storm_category in _profile_caps:
        adjusted_winds = np.minimum(adjusted_winds, _profile_caps[storm_category])
...
def calculate_decay_parameters(elevation, landfall_wind):
    ...
    Enhanced for Philippine mountain barriers: Sierra Madre, Cordillera Central,
    and Visayan ranges cause rapid intensity loss for all super typhoons.
    """
    
    # Base decay rate calibrated for Philippines
    alpha_base = 0.035  # Increased from 0.031 for stronger baseline
    
    # Create continuous relationship between elevation and terrain factor
    # Steeper exponential response — Philippine mountains are narrow but tall
    terrain_factor = 1.0 + 2.0 * (1.0 - np.exp(-elevation / 350.0))
    
    # Scale intensity factor based on storm strength
    # Stronger storms interact more with terrain (larger vortex, more friction)
    if landfall_wind >= 106:  # Super typhoons
        intensity_factor = 2.5 + 0.8 * (landfall_wind - 106) / 20.0
    elif landfall_wind >= 100:  # Near-super typhoons
    ...
    roughness_factor = 1.0 + 0.7 * (1.0 - np.exp(-elevation / 600.0))
    
    # Calculate final decay parameter
    alpha = alpha_base * terrain_factor * intensity_factor * roughness_factor
    
    # Adjust max_alpha — allow stronger decay for STY over mountains
    max_alpha = 0.35 if landfall_wind >= 106 else 0.25 if landfall_wind >= 100 else 0.18
...
def apply_orographic_effects(storm_df, dem_path):
    ...
    Enhanced to include both:
    1. Track deflection along mountain barriers
    2. Wind speed attenuation proportional to terrain height and cross-section
    
    Historical constraint: No super typhoon (>=106 kt) survives crossing
    the Sierra Madre, Cordillera Central, or Visayan mountain ranges.
    """
    modified_df = storm_df.copy()
    
    # Process storms >= 90 kt (Typhoon and above)
    ...
    track_winds = modified_df['WIND'].values.copy()
    
    # ── Wind attenuation through terrain ──
    for i in range(len(track_lats)):
        if track_winds[i] < 64:  # Only process typhoon-strength and above
            continue
            
        lat, lon = track_lats[i], track_lons[i]
        
        if is_over_land(lon, lat):
            elev = get_elevation(lon, lat)
            
            if elev > 200 and track_winds[i] >= 106:
                # Proportional attenuation for extreme storms over elevated terrain
                # Based on empirical observation: mountains strip STY intensity rapidly
                height_factor = min(1.0, elev / 1500.0)  # Normalized to ~1500m (Sierra Madre peaks)
                attenuation = 0.08 + 0.15 * height_factor  # 8% to 23% reduction per timestep
                new_wind = track_winds[i] * (1.0 - attenuation)
                new_wind = max(new_wind, 60.0)  # Floor at strong tropical storm
                
                if new_wind < track_winds[i]:
                    logging.info(
                        f"OROGRAPHIC WIND ATTEN: Point {i} ({lat:.2f}N, {lon:.2f}E) "
                        f"elev={elev:.0f}m, {track_winds[i]:.1f}kt -> {new_wind:.1f}kt"
                    )
                    track_winds[i] = new_wind
    
    modified_df['WIND'] = track_winds
    
    # ── Track deflection (original logic, refined) ──
...
def preserve_eastern_approaches(storm_df):
    """
    REPLACED: Now applies mountain barrier attenuation instead of preserving intensity.
    
    Historical evidence shows that ALL typhoons > 106 kt are attenuated when crossing
    the major mountain ranges of the Philippines:
      - Sierra Madre / Cordillera Central (eastern & central Luzon)
      - Pacific Cordillera of Mindanao (eastern Mindanao)  
      - Visayan mountains (Leyte, Samar, Cebu, Negros, Panay)
    
    No super typhoon (>=106 kt) should emerge on the western side of these barriers
    at its original intensity. This function enforces that physical constraint.
    """
    df = storm_df.copy()
    
    # ── Define Philippine mountain barrier zones ──
    # Each zone: (lon_min, lon_max, lat_min, lat_max, description, max_exit_wind)
    # max_exit_wind: maximum wind speed allowed after crossing the barrier
    MOUNTAIN_BARRIERS = [
        # Sierra Madre — eastern Luzon backbone (tallest continuous range)
        (121.0, 122.8, 14.0, 19.0, "Sierra Madre", 85.0),
        # Cordillera Central — northern Luzon highlands
        ...
        (124.0, 125.5, 7.0, 8.5, "Central Mindanao Highlands", 80.0),
        # Visayas — Leyte/Samar mountains
        (124.0, 125.5, 10.0, 12.5, "Leyte-Samar Mtns", 90.0),
        # Visayas — Cebu/Negros/Panay interior mountains
        (122.5, 124.5, 9.5, 11.5, "Central Visayas Mtns", 90.0),
    ]
    
    max_wind = df['WIND'].max()
    if max_wind < 90:
        return df  # Weaker storms don't need this check
    
    lats = df['LAT'].values
    lons = df['LON'].values
    winds = df['WIND'].values.copy()
    
    # ── Track which barriers the storm has crossed ──
    crossed_barriers = set()
    cumulative_land_hours = 0  # 3-hourly timesteps
    
    for i in range(len(df)):
        lat, lon, wind = lats[i], lons[i], winds[i]
        
        # Check if this point is over land
        point_over_land = is_over_land(lon, lat)
        if point_over_land:
            cumulative_land_hours += 3
        
        # Check each mountain barrier
        for barrier in MOUNTAIN_BARRIERS:
            blon_min, blon_max, blat_min, blat_max, name, max_exit = barrier
            
            if blon_min <= lon <= blon_max and blat_min <= lat <= blat_max:
                if name not in crossed_barriers:
                    crossed_barriers.add(name)
                    logging.info(f"MOUNTAIN BARRIER: Storm entering {name} zone at ({lat:.2f}N, {lon:.2f}E) with {wind:.1f}kt")
                
                if point_over_land and wind > max_exit:
                    # Get elevation for proportional attenuation
                    elev = get_elevation(lon, lat)
                    
                    # Attenuation factor scales with elevation
                    if elev > 1000:
                        # Major mountain: aggressive attenuation
                        atten_factor = 0.65 + 0.10 * np.exp(-elev / 1500.0)
                    elif elev > 500:
                        # Moderate highland: strong attenuation
                        atten_factor = 0.75 + 0.10 * np.exp(-elev / 1000.0)
                    elif elev > 100:
                        # Low hills: moderate attenuation  
                        atten_factor = 0.85 + 0.05 * np.exp(-elev / 500.0)
                    else:
                        # Coastal lowland in barrier zone: mild friction
                        atten_factor = 0.92
                    
                    # Additional penalty for time spent over land
                    time_penalty = min(0.10, cumulative_land_hours * 0.012)
                    atten_factor = max(0.50, atten_factor - time_penalty)
                    
                    new_wind = max(max_exit, wind * atten_factor)
                    
                    if new_wind < wind:
                        logging.info(
                            f"MOUNTAIN ATTEN: {name} at ({lat:.2f}N, {lon:.2f}E), "
                            f"elev={elev:.0f}m, {wind:.1f}kt -> {new_wind:.1f}kt "
                            f"(factor={atten_factor:.3f})"
                        )
                        winds[i] = new_wind
    
    # ── Hard constraint: no ≥106 kt west of the main mountain barriers ──
    # After crossing any east-facing mountain range, cap at barrier max_exit
    if crossed_barriers:
        for i in range(len(df)):
            lon, lat, wind = lons[i], lats[i], winds[i]
            
            # West of Sierra Madre / Cordillera (Luzon interior & west coast)
            if lon < 121.0 and 14.0 <= lat <= 19.0 and wind >= 106:
                winds[i] = min(wind, 90.0)
                logging.info(f"W-LUZON CAP: ({lat:.2f}N, {lon:.2f}E) {wind:.1f}kt -> {winds[i]:.1f}kt")
            
            # West of Visayas mountains
            if lon < 123.0 and 9.5 <= lat <= 12.5 and wind >= 106:
                winds[i] = min(wind, 90.0)
                logging.info(f"W-VISAYAS CAP: ({lat:.2f}N, {lon:.2f}E) {wind:.1f}kt -> {winds[i]:.1f}kt")
            
            # West of Mindanao mountains
            if lon < 124.5 and 6.5 <= lat <= 10.0 and wind >= 106:
                winds[i] = min(wind, 90.0)
                logging.info(f"W-MINDANAO CAP: ({lat:.2f}N, {lon:.2f}E) {wind:.1f}kt -> {winds[i]:.1f}kt")
    
    df['WIND'] = winds
    
    # Recalculate pressure and category
    df['PRES'] = calculate_pressure_from_wind(winds, env_pressure=1010.0, lat=df['LAT'])
...
def enforce_overwater_sty_continuity(wind_speeds, track_lats, track_lons):
    """
    Ensure that ≥106 kt intensity is continuous over open water.
    
    Historical observation: STY storms maintain ≥106 kt as a single unbroken
    stretch over the open Pacific. They do NOT oscillate above/below 106 kt
    intermittently while still over water. Gaps should only appear at landfall
    (terrain attenuation) or during genuine terminal weakening.
    
    Algorithm:
      1. Identify the first and last overwater ≥106 kt indices.
      2. Between those bounds, if a point is over water and <106 kt but is
         flanked (within a window of ±3 points) by overwater ≥106 kt points,
         raise it to 106 kt.
      3. Do NOT touch overland points (those are legitimate attenuation).
    
    Must be called AFTER all wind generation and BEFORE DataFrame creation.
    """
    winds = np.array(wind_speeds, dtype=np.float64)
    n = len(winds)
    if n < 5:
        return winds
    
    # Build overwater mask (True = over open water)
    overwater = np.array([
        not is_over_land(track_lons[i], track_lats[i])
        for i in range(n)
    ])
    
    # Find overwater ≥106 kt indices
    sty_overwater = (winds >= 106.0) & overwater
    sty_indices = np.where(sty_overwater)[0]
    
    if len(sty_indices) < 2:
        return winds  # No sustained STY pattern to enforce
    
    first_sty = sty_indices[0]
    last_sty = sty_indices[-1]
    
    # Fill gaps in the overwater ≥106 envelope
    # A "gap" is an overwater point between first_sty and last_sty where wind < 106
    # but nearby (within ±2 steps) overwater points are ≥106.
    GAP_WINDOW = 2
    filled = 0
    for i in range(first_sty, last_sty + 1):
        if not overwater[i]:
            continue  # Don't touch overland points
        if winds[i] >= 106.0:
            continue  # Already at STY level
        
        # Check if there are ≥106 kt overwater points within GAP_WINDOW on each side
        has_sty_before = any(
            winds[j] >= 106.0 and overwater[j]
            for j in range(max(0, i - GAP_WINDOW), i)
        )
        has_sty_after = any(
            winds[j] >= 106.0 and overwater[j]
            for j in range(i + 1, min(n, i + GAP_WINDOW + 1))
        )
        
        if has_sty_before and has_sty_after:
            winds[i] = 106.0
            filled += 1
    
    if filled > 0:
        logging.info(f"STY PERSISTENCE: Filled {filled} overwater gap points to 106 kt")
...
def enforce_western_intensity_cap(storm_df):
    ...
    After crossing Philippine mountain barriers, no storm should retain
    super typhoon intensity (>=100 kt). Historical record confirms this.
    """
    modified_df = storm_df.copy()
    
    for i, row in modified_df.iterrows():
        lat, lon, wind = row['LAT'], row['LON'], row['WIND']
        
        # West of main Philippine mountain barriers
        is_western_luzon = (lon <= 120.5 and 14.0 <= lat <= 19.0)
        is_western_visayas = (lon <= 123.0 and 9.5 <= lat <= 12.5)
        is_western_mindanao = (lon <= 124.0 and 6.5 <= lat <= 10.0)
        is_south_china_sea = (lon <= 119.0 and 5.0 <= lat <= 21.0)
        
        if (is_western_luzon or is_western_visayas or is_western_mindanao) and wind >= 96:
            # Hard cap at 90kt immediately west of mountain barriers
            modified_df.loc[i, 'WIND'] = 90.0
            modified_df.loc[i, 'CATEGORY'] = 'Typhoon'
            logging.info(f"WESTERN CAP: ({lat:.2f}N, {lon:.2f}E) {wind:.1f}kt -> 90kt")
        
        elif is_south_china_sea and wind >= 106:
            # South China Sea: cap at 95kt (allows some open-water maintenance)
            # NOTE: This applies to all storms transiting the SCS, including
            # Pacific-origin TYs entering the basin. SCS-GENESIS storms are
            # separately constrained by the SCS CATEGORY OVERRIDE in
            # generate_synthetic_storm() which limits ~97% to TD/TS/STS.
            modified_df.loc[i, 'WIND'] = 95.0
            modified_df.loc[i, 'CATEGORY'] = 'Typhoon'
            logging.info(f"SCS CAP: ({lat:.2f}N, {lon:.2f}E) {wind:.1f}kt -> 95kt")
...
def apply_dem_based_decay(storm_df, dem_path):
    ...
    # NOTE: Eastern approach phase preservation REMOVED.
    # Historical data shows ALL super typhoons are attenuated by Philippine mountains.
    # The mountain barrier attenuation is now handled by preserve_eastern_approaches().
    ...
    if landfall_idx is not None:
        ...
        # ENHANCED: Apply immediate friction-based reduction at landfall
        # Mountain barriers cause rapid intensity loss; no STY preservation
        if V0 >= 106:
            # Get elevation at landfall point
            lf_elev = elevation[landfall_idx] if landfall_idx < len(elevation) else 0.0
            
            if lf_elev > 800:
                # Major mountain landfall: immediate strong reduction
                V0_preserved = min(V0, max(85.0, V0 * 0.80))
                logging.info(f"MOUNTAIN LANDFALL: {V0:.1f}kt -> {V0_preserved:.1f}kt (elev={lf_elev:.0f}m)")
            elif lf_elev > 300:
                # Highland landfall: significant reduction
                V0_preserved = min(V0, max(90.0, V0 * 0.85))
                logging.info(f"HIGHLAND LANDFALL: {V0:.1f}kt -> {V0_preserved:.1f}kt (elev={lf_elev:.0f}m)")
            elif lf_elev > 50:
                # Low terrain landfall: moderate reduction
                V0_preserved = min(V0, max(95.0, V0 * 0.90))
                logging.info(f"LAND LANDFALL: {V0:.1f}kt -> {V0_preserved:.1f}kt (elev={lf_elev:.0f}m)")
            else:
                # Coastal/flat landfall: standard friction loss
                V0_preserved = min(V0, max(100.0, V0 * 0.92))
                logging.info(f"COASTAL LANDFALL: {V0:.1f}kt -> {V0_preserved:.1f}kt")
        ...
        if any(elev_land > 400):  # Mountains in track (lowered threshold)
            intensity_factor *= 1.4  # Increased from 1.2 for stronger mountain effect
            
        # Enhanced terrain factors — steeper response for Philippine mountain barriers
        # Sierra Madre, Cordillera Central, and Visayan peaks cause rapid decay
        terrain_factor = 1.0 + 2.2*(1.0 - np.exp(-elev_land/300.0))  # Steeper, lower half-height
        roughness_factor = 1.0 + 0.9*(1.0 - np.exp(-elev_land/400.0))  # Increased roughness
        
        alpha_vals = alpha_base * terrain_factor * intensity_factor * roughness_factor
        
        # Cap α by storm strength — raised for mountain attenuation
        max_alpha = 0.35 if V0>=106 else 0.25 if V0>=100 else 0.18
        ...
        # Mountain disruption for extreme storms — significantly enhanced
        if V0 >= 106:
            # Lower threshold and stronger effect for Philippine mountain ranges
            mask_high = (elev_land > 300.0)
            if mask_high.any():
                # Stronger mountain factor: scales aggressively with elevation
                mf = 1.5 + (elev_land[mask_high] - 300.0) / 400.0
                decayed[mask_high] = Vb + (decayed[mask_high] - Vb) / mf
                
                # Additional hard cap: no point over terrain >500m should retain STY intensity
                mask_very_high = (elev_land > 500.0)
                if mask_very_high.any():
                    decayed[mask_very_high] = np.minimum(decayed[mask_very_high], 85.0)
                    logging.info(f"MOUNTAIN HARD CAP: {np.sum(mask_very_high)} points over 500m capped at 85kt")
...
def generate_genesis_points(df_historical, year, month, count, category=None):
    ...
    if isinstance(df_historical, pd.DataFrame) and not df_historical.empty:
        ...
        try:
            ...
            # Fall back to default genesis points (east of archipelago, over water)
            _fb_ranges = {
                'STY': (12, 18, 132, 138), 'TY': (10, 16, 128, 135),
                'STS': (8, 14, 127, 132),  'TS': (8, 14, 127, 132),
                'TD':  (6, 12, 127, 133),
            }
            _fb = _fb_ranges.get(category, _fb_ranges['TD'])
            for _attempt in range(30):
                _lat = np.random.uniform(_fb[0], _fb[1])
                _lon = np.random.uniform(_fb[2], _fb[3])
                if not is_over_land(_lon, _lat) and not is_in_genesis_exclusion_zone(_lon, _lat):
                    return [(_lat, _lon)]
            return [(_fb[0] + 2, _fb[3] - 1)]  # Last resort: center of range
    ...
    genesis_regions = {
        ...
        # SCS genesis: very rare in Jan, tiny weight for TD/TS only
        1: {
            'TD': [(6.0, 132.0, 1.0, 1.5, 0.5), (8.0, 129.0, 1.2, 1.2, 0.5),
                   (7.0, 117.0, 1.0, 1.5, 0.05)],  # SCS
            'TS': [(7.0, 134.0, 1.0, 1.5, 0.6), (10.0, 132.0, 1.0, 1.2, 0.4),
                   (7.5, 116.0, 1.0, 1.5, 0.03)],  # SCS
        ...
        # SCS genesis: very rare
        2: {
            'TD': [(5.0, 133.0, 1.0, 1.5, 0.6), (8.0, 130.0, 1.0, 1.0, 0.4),
                   (6.5, 117.0, 1.0, 1.5, 0.04)],  # SCS
        ...
        # SCS genesis: rare, only weak systems
        4: {
            'TD': [(7.0, 130.0, 1.5, 1.5, 0.5), (6.0, 135.0, 1.5, 1.5, 0.5),
                   (7.5, 116.5, 1.0, 1.5, 0.05)],  # SCS
        ...
        # SCS genesis: starts to pick up with early SW monsoon
        5: {
            'TD': [(7.0, 129.0, 1.5, 1.5, 0.35), (8.0, 134.0, 1.5, 1.5, 0.55),
                   (8.0, 116.0, 1.5, 1.5, 0.10)],  # SCS
            'TS': [(9.0, 132.0, 1.2, 1.5, 0.45), (11.0, 136.0, 1.0, 1.5, 0.45),
                   (9.0, 117.0, 1.5, 1.5, 0.10)],  # SCS
            'STS': [(10.0, 134.0, 1.0, 1.5, 0.50), (12.0, 138.0, 1.0, 1.5, 0.50)],
            'TY': [(10.5, 136.0, 1.0, 1.5, 0.6), (13.0, 138.0, 1.0, 1.0, 0.4)],
            'STY': [(11.0, 138.0, 1.0, 1.0, 0.7), (14.0, 140.0, 1.0, 1.0, 0.3)]
        },
        # June: Eastern and Northern pattern + SW monsoon active
        # SCS genesis: moderate — monsoon trough extends into SCS
        6: {
            'TD': [(9.0, 130.0, 1.5, 1.5, 0.25), (11.0, 134.0, 1.5, 1.5, 0.60),
                   (9.0, 116.0, 1.5, 1.5, 0.08), (13.0, 115.5, 1.5, 1.5, 0.07)],  # SCS low-lat + mid-lat
            'TS': [(10.0, 132.0, 1.2, 1.5, 0.30), (13.0, 135.0, 1.0, 1.5, 0.50),
                   (10.0, 117.0, 1.5, 1.5, 0.10), (14.0, 116.0, 1.5, 1.5, 0.10)],  # SCS
            'STS': [(11.0, 134.0, 1.0, 1.5, 0.40), (14.0, 137.0, 1.0, 1.5, 0.60)],
            'TY': [(12.0, 136.0, 1.0, 1.5, 0.40), (15.0, 138.0, 1.0, 1.0, 0.60)],
            'STY': [(13.0, 138.0, 1.0, 1.0, 0.5), (16.0, 139.0, 1.0, 1.0, 0.5)]
            # No SCS genesis for STY — physically unrealistic
        },
        # July: Strong Northern pattern + peak SW monsoon
        # SCS genesis: significant — monsoon trough at its deepest
        7: {
            'TD': [(10.0, 129.0, 1.5, 1.5, 0.20), (14.0, 133.0, 1.5, 1.5, 0.60),
                   (9.0, 116.0, 1.5, 1.5, 0.08), (14.0, 115.5, 2.0, 1.5, 0.12)],  # SCS
            'TS': [(12.0, 131.0, 1.2, 1.5, 0.20), (16.0, 134.0, 1.0, 1.5, 0.60),
                   (10.0, 117.0, 1.5, 1.5, 0.08), (15.0, 116.0, 2.0, 1.5, 0.12)],  # SCS
            'STS': [(13.0, 132.0, 1.0, 1.5, 0.30), (17.0, 135.0, 1.0, 1.5, 0.70)],
            'TY': [(14.0, 134.0, 1.0, 1.5, 0.30), (18.0, 136.0, 1.0, 1.0, 0.70)],
            'STY': [(15.0, 135.0, 1.0, 1.0, 0.3), (19.0, 137.0, 1.0, 1.0, 0.7)]
        },
        # August: Strong Northern pattern + peak SW monsoon
        # SCS genesis: significant
        8: {
            'TD': [(11.0, 128.0, 1.5, 1.5, 0.20), (16.0, 132.0, 1.5, 1.5, 0.60),
                   (9.0, 116.0, 1.5, 1.5, 0.08), (15.0, 115.5, 2.0, 1.5, 0.12)],  # SCS
            'TS': [(13.0, 130.0, 1.2, 1.5, 0.20), (17.0, 134.0, 1.0, 1.5, 0.60),
                   (10.0, 117.0, 1.5, 1.5, 0.08), (16.0, 116.0, 2.0, 1.5, 0.12)],  # SCS
            'STS': [(14.0, 132.0, 1.0, 1.5, 0.30), (18.0, 135.0, 1.0, 1.5, 0.70)],
            'TY': [(15.0, 133.0, 1.0, 1.5, 0.30), (19.0, 136.0, 1.0, 1.0, 0.70)],
            'STY': [(16.0, 134.0, 1.0, 1.0, 0.3), (20.0, 137.0, 1.0, 1.0, 0.7)]
        },
        # September: Very strong Northern pattern + monsoon still active
        # SCS genesis: secondary peak — monsoon trough very active
        9: {
            'TD': [(12.0, 127.0, 1.5, 1.5, 0.15), (17.0, 131.0, 1.5, 1.5, 0.65),
                   (8.0, 116.0, 1.5, 1.5, 0.08), (14.0, 115.5, 2.0, 1.5, 0.12)],  # SCS
            'TS': [(14.0, 129.0, 1.2, 1.5, 0.15), (18.0, 133.0, 1.0, 1.5, 0.65),
                   (9.0, 117.0, 1.5, 1.5, 0.08), (15.0, 116.0, 2.0, 1.5, 0.12)],  # SCS
            'STS': [(15.0, 131.0, 1.0, 1.5, 0.20), (19.0, 134.0, 1.0, 1.5, 0.80)],
            'TY': [(16.0, 132.0, 1.0, 1.5, 0.20), (20.0, 135.0, 1.0, 1.0, 0.80)],
            'STY': [(17.0, 133.0, 1.0, 1.0, 0.2), (21.0, 136.0, 1.0, 1.0, 0.8)]
        },
        # October: Northern and Eastern pattern + monsoon winding down
        # SCS genesis: moderate — still active but decreasing
        10: {
            'TD': [(10.0, 129.0, 1.5, 1.5, 0.22), (15.0, 132.0, 1.5, 1.5, 0.60),
                   (8.0, 116.0, 1.5, 1.5, 0.08), (13.0, 115.5, 2.0, 1.5, 0.10)],  # SCS
            'TS': [(12.0, 131.0, 1.2, 1.5, 0.22), (16.0, 134.0, 1.0, 1.5, 0.60),
                   (9.0, 117.0, 1.5, 1.5, 0.08), (14.0, 116.0, 2.0, 1.5, 0.10)],  # SCS
            'STS': [(13.0, 133.0, 1.0, 1.5, 0.30), (17.0, 135.0, 1.0, 1.5, 0.70)],
            'TY': [(14.0, 134.0, 1.0, 1.5, 0.30), (18.0, 136.0, 1.0, 1.0, 0.70)],
            'STY': [(15.0, 135.0, 1.0, 1.0, 0.3), (19.0, 137.0, 1.0, 1.0, 0.7)]
        },
        # November: Shifting back to Central and Eastern
        # SCS genesis: decreasing but still present as NE monsoon surges begin
        11: {
            'TD': [(8.0, 130.0, 1.5, 1.5, 0.30), (12.0, 133.0, 1.5, 1.5, 0.55),
                   (7.5, 116.0, 1.5, 1.5, 0.08), (11.0, 115.5, 1.5, 1.5, 0.07)],  # SCS
            'TS': [(9.0, 132.0, 1.2, 1.5, 0.32), (13.0, 135.0, 1.0, 1.5, 0.55),
                   (8.5, 117.0, 1.5, 1.5, 0.06), (12.0, 116.0, 1.5, 1.5, 0.07)],  # SCS
            'STS': [(10.0, 134.0, 1.0, 1.5, 0.40), (14.0, 137.0, 1.0, 1.5, 0.60)],
            'TY': [(11.0, 136.0, 1.0, 1.5, 0.40), (15.0, 138.0, 1.0, 1.0, 0.60)],
            'STY': [(12.0, 137.0, 1.0, 1.0, 0.4), (16.0, 139.0, 1.0, 1.0, 0.6)]
        },
        # December: Eastern pattern
        # SCS genesis: low — NE monsoon shear suppresses development
        12: {
            'TD': [(7.0, 131.0, 1.5, 1.5, 0.35), (9.0, 134.0, 1.5, 1.5, 0.55),
                   (7.0, 117.0, 1.0, 1.5, 0.05), (9.0, 116.0, 1.0, 1.5, 0.05)],  # SCS
            'TS': [(8.0, 133.0, 1.2, 1.5, 0.45), (10.0, 136.0, 1.0, 1.5, 0.45),
                   (8.0, 117.0, 1.0, 1.5, 0.05), (10.0, 116.0, 1.0, 1.5, 0.05)],  # SCS
    ...
    # ——— Inject Sulu Sea TD genesis center (conservative) ———
    # Target: ~0.39 TD/yr in Sulu box (IBTrACS 1923-2023).
    # Center at (9.0°N, 120.5°E) — IHO Sulu Sea deep basin centroid.
    # Spread σ=(0.7°, 0.6°) keeps ~69% of samples inside the 
    # 7–9.5°N, 119.5–121.5°E zone. Overshoot is caught by the exclusion polygon.
    _sulu_td_weights = {
        1: 0.005, 2: 0.005, 3: 0.005, 4: 0.005,  # off-season
        5: 0.02,                                     # transition
        6: 0.04, 7: 0.04, 8: 0.04, 9: 0.04,        # SW monsoon
        10: 0.03, 11: 0.02, 12: 0.01                 # winding down
    }
    for _m in range(1, 13):
        _w = _sulu_td_weights.get(_m, 0.005)
        if _m in genesis_regions and 'TD' in genesis_regions[_m]:
            genesis_regions[_m]['TD'].append(
                (9.0, 120.5, 0.7, 0.6, _w)  # Sulu Sea deep basin centroid
            )
    ...
    while len(sampled_points) < count and total_attempts < max_attempts * count:
        ...
        # Reject genesis in Philippine archipelago zone (inter-island seas)
        if is_in_genesis_exclusion_zone(perturbed_lon, perturbed_lat):
            # HARD BLOCK: Palawan–Mindoro gap and Visayan Sea (119–122°E, 10–14.5°N)
            # Catches the ~120°E, 12°N hotspot BEFORE the SCS fringe check
            if is_in_palawan_mindoro_gap(perturbed_lon, perturbed_lat):
                total_attempts += 1
                continue
            # Allow only TD/TS in open SCS west of Palawan (lon < 119°E)
            # FIX-D: with Borneo geographic floor
            elif perturbed_lon < 119.0 and category in ('TD', 'TS'):
                if perturbed_lat < _SCS_MIN_LAT:
                    total_attempts += 1
                    continue
                # else: pass through — weak monsoon system
            # Allow TD-only in tight central Sulu Sea basin (7–9.5°N, 119.5–121.5°E)
            elif (category == 'TD'
                  and is_in_sulu_sea_genesis_zone(perturbed_lon, perturbed_lat)
                  and not is_over_land(perturbed_lon, perturbed_lat)):
                pass  # Rare weak-TD genesis — historically ~0.39/yr
            else:
                total_attempts += 1
                continue
        # FIX-D: Even outside the exclusion polygon, block genesis near Borneo
        elif perturbed_lon < _SCS_MIN_LON and perturbed_lat < _SCS_MIN_LAT:
            total_attempts += 1
            continue
    ...
    if len(sampled_points) < count:
        ...
        for _ in range(missing_count):
            ...
            for _ in range(10):  # Try a few times for each missing point
                ...
                if (is_inside_par(new_lat, new_lon, use_buffer=True) 
                    and not is_over_land(new_lon, new_lat) 
                    and not is_in_genesis_exclusion_zone(new_lon, new_lat)):
...
def generate_track_dynamics(initial_lat, initial_lon, start_month, duration=48, track_coef=None, 
    ...
    for i in range(1, duration):
        ...
        # ——— LOW-LATITUDE TRADE-WIND ENFORCEMENT (v5) ———
        # Below ~8°N, the deep tropical easterlies dominate steering.
        # TCs at these latitudes move WNW with very little meridional wobble.
        # Without this, the James-Mason model + data blend can produce
        # unrealistic eastward or flat zonal tracks.
        if current_lat < 8.0:
            # Strength of trade-wind correction increases toward equator
            trade_strength = 1.0 - (current_lat - 4.0) / 4.0  # 1.0 at 4°N, 0.0 at 8°N
            trade_strength = np.clip(trade_strength, 0.0, 1.0)
            
            # Enforce westward motion: if dlon is positive (eastward), strongly damp it
            if dlon1 > 0:
                dlon1 *= (1.0 - 0.9 * trade_strength)  # Kill 90% of eastward at 4°N
            
            # Add persistent westward push (trade winds ~10-15 kt at these latitudes)
            dlon1 -= 0.15 * trade_strength * np.random.uniform(0.8, 1.2)
            
            # Ensure minimum northward component (beta drift)
            ...
            max_dlon_east = max_dlon * 0.8  # Original v4 behavior
        
        dlat1 = np.clip(dlat1, -max_dlat, max_dlat)
        dlon1 = np.clip(dlon1, -max_dlon, max_dlon_east)
...
def generate_synthetic_storm(year, month, historical_positions=None, historical_winds=None, used_ids=None):
    ...
    if historical_positions is not None and not historical_positions.empty:
        ...
        if genesis_points:
            ...
            # Fallback to default genesis points — always over open ocean east of archipelago
            logging.warning(f"No valid genesis points for {year}-{month}, category {storm_category}, using defaults")
            # Use category-specific defaults (all east of 126°E to avoid archipelago)
            # Retry loop ensures the point is over water and outside the exclusion zone
            _fallback_ranges = {
                'STY': (12, 18, 132, 138), 'TY': (10, 16, 128, 135),
                'STS': (8, 14, 127, 132),  'TS': (8, 14, 127, 132),
                'TD':  (6, 12, 127, 133),
            }
            _fb = _fallback_ranges.get(storm_category, _fallback_ranges['TD'])
            for _attempt in range(30):
                initial_lat = np.random.uniform(_fb[0], _fb[1])
                initial_lon = np.random.uniform(_fb[2], _fb[3])
                if not is_over_land(initial_lon, initial_lat) and not is_in_genesis_exclusion_zone(initial_lon, initial_lat):
                    break
        
        # ENHANCEMENT: Bias towards eastern genesis for better PAR entry
        # Skip for Sulu Sea TD genesis to preserve intentional placement
        _is_sulu_td = (storm_category == 'TD'
                       and is_in_sulu_sea_genesis_zone(initial_lon, initial_lat))
        if not _is_sulu_td and np.random.random() < 0.5:
        ...
        # FINAL SAFETY NET: Ensure genesis point is not over land or inside the exclusion zone
        # Exception: Sulu Sea TD genesis is allowed through
        _in_excl = is_in_genesis_exclusion_zone(initial_lon, initial_lat)
        ...
        if is_over_land(initial_lon, initial_lat, buffer_km=30) or (_in_excl and not _sulu_ok):
            ...
            for _attempt in range(20):
                ...
                initial_lat = np.random.uniform(8.0, 16.0)
                if not is_over_land(initial_lon, initial_lat) and not is_in_genesis_exclusion_zone(initial_lon, initial_lat):
                    break
            logging.debug(f"Genesis safety net triggered — relocated to ({initial_lat:.2f}N, {initial_lon:.2f}E)")
    
    else:
        # Default genesis point generation with category specificity
        # All longitudes kept east of 126°E to avoid Philippine archipelago interior
        # Retry loop ensures the point is over water and outside the exclusion zone
        _fallback_ranges = {
            'STY': (12, 18, 132, 138), 'TY': (10, 16, 128, 135),
            'STS': (8, 14, 127, 132),  'TS': (8, 14, 127, 132),
        ...
        # SAFETY NET: Ensure genesis point is not over land or inside the exclusion zone
        if is_over_land(initial_lon, initial_lat) or is_in_genesis_exclusion_zone(initial_lon, initial_lat):
            for _attempt in range(20):
                initial_lon = np.random.uniform(127.0, 133.0)
                initial_lat = np.random.uniform(8.0, 16.0)
                if not is_over_land(initial_lon, initial_lat) and not is_in_genesis_exclusion_zone(initial_lon, initial_lat):
                    break
            logging.debug(f"Genesis safety net triggered (no-hist branch) — relocated to ({initial_lat:.2f}N, {initial_lon:.2f}E)")
    
    # ——— SCS GENESIS CATEGORY OVERRIDE (IBTrACS 1977–2023 calibrated) ———
    # SCS-born storms (genesis lon ≤ 120°E, lat 5–25°N) are overwhelmingly
    # weak: 100% are TD or weaker at genesis, only ~10% ever reach STS+,
    # and ~2% reach TY+ in their lifetime (IBTrACS 1977–2023, N=177).
    # The PAR-wide category distribution vastly over-intensifies SCS storms.
    # Override category here AFTER genesis is placed but BEFORE track/wind
    # profile generation, so the entire downstream intensity model is correct.
    #
    # SCS lifetime-max intensity distribution (IBTrACS-calibrated):
    #   TD: 43%, TS: 45%, STS: 10%, TY: 2%, STY: 0%
    _is_scs_genesis = (initial_lon <= 120.0 and 5.0 <= initial_lat <= 25.0
                       and not is_in_genesis_exclusion_zone(initial_lon, initial_lat))
    if _is_scs_genesis and storm_category in ('STS', 'TY', 'STY'):
        _scs_roll = random.random() * 100.0
        if _scs_roll <= 43.0:
            _old_cat = storm_category
            storm_category = 'TD'
            logging.info(f"SCS CATEGORY OVERRIDE: {_old_cat} → TD "
                         f"(genesis {initial_lat:.1f}N, {initial_lon:.1f}E)")
        elif _scs_roll <= 88.0:
            _old_cat = storm_category
            storm_category = 'TS'
            logging.info(f"SCS CATEGORY OVERRIDE: {_old_cat} → TS "
                         f"(genesis {initial_lat:.1f}N, {initial_lon:.1f}E)")
        elif _scs_roll <= 98.0:
            _old_cat = storm_category
            storm_category = 'STS'
            logging.info(f"SCS CATEGORY OVERRIDE: {_old_cat} → STS "
                         f"(genesis {initial_lat:.1f}N, {initial_lon:.1f}E)")
        else:
            # ~2% chance: keep TY (but never STY in SCS)
            if storm_category == 'STY':
                storm_category = 'TY'
                logging.info(f"SCS CATEGORY OVERRIDE: STY → TY "
                             f"(genesis {initial_lat:.1f}N, {initial_lon:.1f}E)")
            else:
                logging.info(f"SCS CATEGORY RARE TY: kept at TY "
                             f"(genesis {initial_lat:.1f}N, {initial_lon:.1f}E)")

    # ——— SCS GENESIS: Set wind cap for downstream clamping ———
    _scs_wind_cap = None  # None = no SCS constraint (Pacific-origin storm)
    if _is_scs_genesis:
        _scs_wind_cap = _SCS_CATEGORY_WIND_CAPS.get(storm_category, 47.0)
        logging.info(f"SCS GENESIS: wind cap set to {_scs_wind_cap:.0f} kt "
                     f"for category {storm_category}")
    ...
    # ——— FIX: Cap initial IID draws to assigned category ceiling ———
    # Without this, TD/TS/STS storms get uncapped weibull_gev_blend draws
    # that routinely exceed 100 kt, inflating STY track-point counts.
    # (Continuation segments already have this cap at line ~4998.)
    #_init_cat_caps = {'TD': 33.0, 'TS': 47.0, 'STS': 63.0, 'TY': 99.0, 'STY': 126.0}
    #_init_cap = _init_cat_caps.get(storm_category, 126.0)
    #initial_wind_speeds = np.minimum(initial_wind_speeds, _init_cap)
    
    # Generate initial wind speeds — CATEGORY-AWARE
    if storm_category in ('STY', 'TY'):
        if historical_winds is not None and len(historical_winds) > 10:
            initial_wind_speeds = weibull_gev_blend(len(initial_track_lats), historical_winds, month, region="PAR", year=year)
        else:
        ...
        if historical_winds is not None and len(historical_winds) > 10:
            initial_wind_speeds = sample_from_historical_distribution(
                len(initial_track_lats), historical_winds, category=storm_category
            )
        else:
            _cat_bounds = {'TD': (22.0, 33.0), 'TS': (34.0, 47.0), 'STS': (48.0, 63.0)}
            _lo, _hi = _cat_bounds.get(storm_category, (22.0, 33.0))
            initial_wind_speeds = np.random.uniform(_lo, _hi, len(initial_track_lats)).astype(np.float32)
    
    initial_wind_speeds = validate_wind_speeds(initial_wind_speeds, caller="generate_synthetic_storm-initial")
    
    # ——— CATEGORY-AWARE WIND PROFILES (STY and TY) ———
    # For STY/TY: reshape the IID draws into a smooth ramp-up / sustained
    # peak / ramp-down profile using adjust_wind_profile.  This ensures
    # extreme storms maintain coherent intensity over water instead of
    # the choppy IID-per-timestep appearance.
    # TD/TS/STS keep the original IID architecture — their "typical" tracks
    # are visually fine and physical attenuation handles their final category.
    if storm_category == 'STY':
        target_peak = sample_category_peak_wind('STY')
        initial_wind_speeds = adjust_wind_profile(
            initial_wind_speeds, target_peak, 'STY'
        )
        initial_wind_speeds = validate_wind_speeds(
            initial_wind_speeds, caller="generate_synthetic_storm-STY-profile"
        )
        logging.info(f"STY PROFILE: beta peak={target_peak:.1f} kt, "
                     f"profile max={max(initial_wind_speeds):.1f} kt")

    elif storm_category == 'TY':
        target_peak = sample_category_peak_wind('TY')
        initial_wind_speeds = adjust_wind_profile(
            initial_wind_speeds, target_peak, 'TY'
        )
        initial_wind_speeds = validate_wind_speeds(
            initial_wind_speeds, caller="generate_synthetic_storm-TY-profile"
        )
        logging.info(f"TY PROFILE: beta peak={target_peak:.1f} kt, "
                     f"profile max={max(initial_wind_speeds):.1f} kt")

    # ——— SCS GENESIS: Clamp initial wind profile to category ceiling ———
    if _scs_wind_cap is not None:
        _pre_clamp_max = max(initial_wind_speeds)
        initial_wind_speeds = np.minimum(initial_wind_speeds, _scs_wind_cap)
        # SCS genesis: IBTrACS shows 100% are TD or weaker at genesis
        initial_wind_speeds[0] = min(initial_wind_speeds[0], 30.0)
        logging.info(f"SCS WIND CLAMP (initial): {_pre_clamp_max:.1f} -> "
                     f"{max(initial_wind_speeds):.1f} kt (cap={_scs_wind_cap:.0f})")
    ...
    while current_duration < max_duration:
        ...
        last_wind = ensure_scalar(wind_speeds[-1])
        _over_water = not is_over_land(next_segment_start_lon, next_segment_start_lat)

        # Track whether storm has made landfall (affects post-landfall ceiling)
        if not _over_water:
            has_made_landfall = True

        # STY/TY over water with strong wind: evolve from last known wind
        # instead of fresh IID draws — maintains smooth intensity over ocean
        if _over_water and last_wind > 50 and storm_category in ('STY', 'TY'):
            next_winds = np.zeros(len(next_lats), dtype=np.float32)
            w = last_wind
            # After landfall, enforce stronger weakening over water
            # (storm structure is disrupted — it cannot re-intensify to STY)
            if has_made_landfall:
                _decay_lo, _decay_hi = 0.94, 0.98  # faster weakening
                _post_landfall_cap = 95.0           # can't recover to STY
            else:
                _decay_lo, _decay_hi = 0.96, 1.00  # open-ocean evolution, no upward drift
                _post_landfall_cap = 126.0
            for _j in range(len(next_lats)):
                _prev_w = w
                w *= np.random.uniform(_decay_lo, _decay_hi)
                # FIX: Hysteresis — if storm was solidly ≥112 kt and the random
                # walk dipped below 106, hold at 106 instead of oscillating.
                # Threshold at 112 (not 106) so marginal STYs can naturally weaken.
                if not has_made_landfall and _prev_w >= 112.0 and w < 106.0:
                    w = 106.0
                w = max(22.0, min(w, _post_landfall_cap))
                next_winds[_j] = w
        else:
            if last_wind >= 100:
                _dyn_cat = 'STY'
                ...
                _dyn_cat = 'TD'
            _cat_order = ['TD', 'TS', 'STS', 'TY', 'STY']
            _eff_cat = _cat_order[min(_cat_order.index(_dyn_cat), 
                                      _cat_order.index(storm_category))]
            
            if historical_winds is not None and len(historical_winds) > 10:
                next_winds = sample_from_historical_distribution(
                    len(next_lats), historical_winds, category=_eff_cat
                )
            else:
                _cat_bounds = {'TD': (22.0, 33.0), 'TS': (34.0, 47.0), 
                               'STS': (48.0, 63.0), 'TY': (64.0, 99.0), 'STY': (100.0, 126.0)}
                _lo, _hi = _cat_bounds.get(_eff_cat, (22.0, 33.0))
                next_winds = np.random.uniform(_lo, _hi, len(next_lats)).astype(np.float32)
                
        next_winds = validate_wind_speeds(next_winds, caller="generate_synthetic_storm-segment")
        
        # ——— SCS GENESIS: Clamp segment winds to category ceiling ———
        if _scs_wind_cap is not None:
            next_winds = np.minimum(next_winds, _scs_wind_cap)
        ...
        try:
            ...
            for i in range(len(next_winds)):
                ...
                # ——— OVER-LAND ATTENUATION ———
                # The Philippines acts as a natural filter for extreme TCs.
                # NO storm maintains ≥106 kt over Philippine land.
                if is_over_land(current_lon, current_lat):
                    has_made_landfall = True
                    elev = get_elevation(current_lon, current_lat)
                    
                    if current_wind >= 106:
                        # Strong attenuation — elevation-scaled
                        if elev > 500:
                            next_winds[i] = min(75.0, current_wind * 0.60)
                        elif elev > 100:
                            next_winds[i] = min(85.0, current_wind * 0.70)
                        else:
                            next_winds[i] = min(90.0, current_wind * 0.75)
                        logging.info(f"LAND ATTEN: ({current_lat:.2f}N, {current_lon:.2f}E) "
                                     f"elev={elev:.0f}m, {current_wind:.1f}kt -> {next_winds[i]:.1f}kt")
                    
                    elif current_wind > 64:
                        # Moderate attenuation for TY-strength over land
                        if elev > 500:
                            next_winds[i] = min(55.0, current_wind * 0.70)
                        elif elev > 100:
                            next_winds[i] = min(65.0, current_wind * 0.80)
                        else:
                            next_winds[i] = current_wind * 0.85
                    
                    elif current_wind > 34:
                        # Light attenuation for TS-strength
                        next_winds[i] = current_wind * 0.90
    ...
    # ── FIX: Enforce continuous ≥106 kt over open water ──
    # Historical STY storms sustain ≥106 kt as a single unbroken stretch
    # over the Pacific; gaps should only appear at landfall.
    if storm_category == 'STY' and max(wind_speeds) >= 106:
        try:
            wind_speeds = enforce_overwater_sty_continuity(
                wind_speeds, track_lats, track_lons
            )
        except Exception as e:
            logging.error(f"Error in overwater STY continuity: {e}")
        
    # Preservation of STY category
    # Calculate if this storm should be an STY based on monthly distribution
    # v7p1: Skip for SCS-genesis storms to prevent re-intensification
    if _scs_wind_cap is None and (storm_category == 'STY' or (month in _monthly_category_dist and 
    ...
    storm_df = pd.DataFrame({
        ...
        'RMW': rmw_values,
        'CATEGORY': categories,
        'SCS_GENESIS': _is_scs_genesis,  # v7p1: propagation flag for downstream clamping
    })
    ...
    if any([is_over_land(lon, lat) for lon, lat in zip(track_lons, track_lats)]):
        ...
        try:
            ...
            logging.error(f"Error in preserve_eastern_approaches for {storm_id}: {e}")
        ...
        try:
            ...
            logging.error(f"Error applying boundary decay: {e}")
            ...
            if current_wind > 90:
                if land_duration == 1:  # First timestep over land (t+0h)
                    ...
                    if current_wind_scalar >= 106:
                        # STY: immediate friction reduction (no longer exempt)
                        new_wind = max(90, current_wind * 0.88)  # ~12% reduction
                    else:
                        new_wind = max(90, current_wind * 0.92)  # 8% reduction
    ...
    # Geographic intensity caps after crossing mountain barriers
    for i, row in storm_df.iterrows():
        lon, lat, wind = row['LON'], row['LAT'], row['WIND']
        
        # West of Luzon mountains (lon < 120.5): no STY allowed
        if lon < 120.5 and 14.0 <= lat <= 19.0 and wind >= 100:
            storm_df.loc[i, 'WIND'] = min(wind, 90.0)
            logging.info(f"FINAL W-LUZON CAP: ({lat:.2f}N, {lon:.2f}E) {wind:.1f}kt -> 90kt")
        
        # West of Visayas (lon < 123): no STY allowed
        elif lon < 123.0 and 9.5 <= lat <= 12.5 and wind >= 100:
            storm_df.loc[i, 'WIND'] = min(wind, 90.0)
            logging.info(f"FINAL W-VISAYAS CAP: ({lat:.2f}N, {lon:.2f}E) {wind:.1f}kt -> 90kt")
        
        # West of Mindanao (lon < 124): no STY allowed  
        elif lon < 124.0 and 6.5 <= lat <= 10.0 and wind >= 100:
            storm_df.loc[i, 'WIND'] = min(wind, 90.0)
            logging.info(f"FINAL W-MINDANAO CAP: ({lat:.2f}N, {lon:.2f}E) {wind:.1f}kt -> 90kt")
        
        # General: nothing ≥106kt west of 120°E over Philippine latitudes
        elif lon < 120.0 and 5.0 <= lat <= 21.0 and wind >= 106:
            storm_df.loc[i, 'WIND'] = 95.0
            logging.info(f"FINAL WEST CAP: ({lat:.2f}N, {lon:.2f}E) {wind:.1f}kt -> 95kt")
    
    # ——— ARCHIPELAGO FILTER: Unconditional over-land intensity cap ———
    # This is the last-resort safety net. Regardless of whether the DEM-based
    # decay succeeded or failed, NO point over Philippine land should have
    # wind ≥ 96 kt. Historical record confirms this: the Philippines acts as
    # a natural filter that attenuates all extreme TCs.
    _land_cap_count = 0
    for i, row in storm_df.iterrows():
        if is_over_land(row['LON'], row['LAT']):
            if row['WIND'] >= 96:
                old_w = row['WIND']
                storm_df.loc[i, 'WIND'] = 85.0
                _land_cap_count += 1
            elif row['WIND'] >= 64:
                # TY-strength over land: cap at 80 kt for mountain terrain
                elev = get_elevation(row['LON'], row['LAT'])
                if elev > 300:
                    storm_df.loc[i, 'WIND'] = min(row['WIND'], 65.0)
    if _land_cap_count > 0:
        logging.info(f"ARCHIPELAGO FILTER: Capped {_land_cap_count} over-land points to ≤85 kt")
    
    # ——— POST-LANDFALL OVER-WATER CAP ———
    # After crossing the Philippines, a storm cannot recover to STY.
    _crossed_land = False
    for i, row in storm_df.iterrows():
        if is_over_land(row['LON'], row['LAT']):
            _crossed_land = True
        elif _crossed_land and row['WIND'] >= 100:
            # Post-landfall, over water, still at STY → cap at 95 kt
            storm_df.loc[i, 'WIND'] = 95.0

    # ——— FINAL GENESIS GATE: No storm may begin over land or inside the archipelago ———
    # FIX-B: Delta-shift the ENTIRE track instead of relocating only point 0.
    if storm_df is not None and not storm_df.empty:
        gen_lon = storm_df.iloc[0]['LON']
        ...
        _sulu_pass = (storm_category == 'TD'
                      and gen_wind < 34.0
                      and not _on_land
                      and _in_excl
                      and is_in_sulu_sea_genesis_zone(gen_lon, gen_lat)
                      and not is_in_palawan_mindoro_gap(gen_lon, gen_lat))

        if _sulu_pass:
            logging.info(f"SULU TD GENESIS: ({gen_lat:.2f}N, {gen_lon:.2f}E) {gen_wind:.0f}kt — allowed through gate")
        elif _on_land or _in_excl:
            for _gate_attempt in range(50):
                new_lon = np.random.uniform(127.0, 135.0)
                new_lat = np.random.uniform(8.0, 18.0)
                if not is_over_land(new_lon, new_lat) and not is_in_genesis_exclusion_zone(new_lon, new_lat):
                    # FIX-B: Shift the ENTIRE track, not just the genesis point
                    dlat = new_lat - gen_lat
                    dlon = new_lon - gen_lon
                    storm_df['LAT'] = storm_df['LAT'] + dlat
                    storm_df['LON'] = storm_df['LON'] + dlon
                    logging.debug(f"FINAL GENESIS GATE: Delta-shifted by ({dlat:+.2f}°, {dlon:+.2f}°) — "
                                  f"genesis now at ({new_lat:.2f}N, {new_lon:.2f}E)")
                    break
    
    # ——— DIAGNOSTIC: Log any storm still born in the exclusion zone ———
    if storm_df is not None and not storm_df.empty:
        g_lon = storm_df.iloc[0]['LON']
        g_lat = storm_df.iloc[0]['LAT']
        if 117.0 <= g_lon <= 126.5 and 5.0 <= g_lat <= 14.5:
            _in_sulu = is_in_sulu_sea_genesis_zone(g_lon, g_lat)
            logging.warning(f"DIAGNOSTIC: Storm born at ({g_lat:.2f}N, {g_lon:.2f}E) — "
                           f"is_over_land={is_over_land(g_lon, g_lat)}, "
                           f"is_in_exclusion={is_in_genesis_exclusion_zone(g_lon, g_lat)}, "
                           f"sulu_zone={_in_sulu}")
...
def post_process_synthetic_tracks(yearly_storms, year):
    ...
    for sid in yearly_storms['SID'].unique():
        ...
        # ——— v5p5 CRITICAL FIX: Re-check genesis after all shifts ———
        # Post-processing shifts can move genesis points over land or into
        # the exclusion zone (e.g. +5° lat shift puts 8°N,121°E → 13°N,121°E
        # = over Panay). Reject the shift if the new genesis is invalid.
        shifted_gen_lat = modified_data['LAT'].iloc[0]
        shifted_gen_lon = modified_data['LON'].iloc[0]
        _shifted_on_land = is_over_land(shifted_gen_lon, shifted_gen_lat)
        _shifted_in_excl = is_in_genesis_exclusion_zone(shifted_gen_lon, shifted_gen_lat)
        # Allow Sulu TD exception
        _shifted_sulu_ok = (category == 'TD'
                            and _shifted_in_excl
                            and is_in_sulu_sea_genesis_zone(shifted_gen_lon, shifted_gen_lat)
                            and not _shifted_on_land)
        if _shifted_on_land or (_shifted_in_excl and not _shifted_sulu_ok):
            # Revert to the un-shifted original track
            logging.debug(f"POST-PROCESS REVERT: Shift moved genesis to "
                         f"({shifted_gen_lat:.2f}N, {shifted_gen_lon:.2f}E) — "
                         f"over_land={_shifted_on_land}, in_excl={_shifted_in_excl}. "
                         f"Keeping original track.")
            modified_data = storm_data.copy()
...
def clamp_scs_genesis_intensity(storms_df):
    """
    FINAL-PASS safety net (v7p1): Enforce IBTrACS-calibrated lifetime-max
    intensity for all storms flagged as SCS_GENESIS.
    
    Must run AFTER all other processing (scale_winds_to_target_mean,
    clean_remnant_low_tracks, etc.) to catch any re-intensification
    from global wind scaling.
    
    IBTrACS 1977–2023 SCS genesis lifetime-max distribution:
        TD  (22–33 kt): 43%
        TS  (34–47 kt): 45%
        STS (48–63 kt): 10%
        TY  (64–95 kt):  2%
    ...
    Strategy: For each SCS-genesis storm whose actual max wind exceeds the
    IBTrACS-drawn category cap, scale the ENTIRE wind profile down
    proportionally (preserving ramp-up / peak / decay shape).
    """
    if 'SCS_GENESIS' not in storms_df.columns:
        logging.warning("SCS_GENESIS column not found — skipping final SCS clamp")
        return storms_df
    
    df = storms_df.copy()
    scs_sids = df.loc[df['SCS_GENESIS'] == True, 'SID'].unique()
    ...
    for sid in scs_sids:
        mask = df['SID'] == sid
        storm_winds = df.loc[mask, 'WIND']
        current_max = storm_winds.max()
        
        # Draw lifetime-max category from IBTrACS SCS distribution
        _roll = random.random() * 100.0
        if _roll <= 43.0:
            cap = 33.0    # TD
        elif _roll <= 88.0:
            cap = 47.0    # TS
            ...
            cap = 95.0    # TY (never STY)
        
        if current_max > cap:
            # Scale the wind profile down proportionally rather than hard-clipping,
            # to preserve the realistic shape (ramp-up / peak / decay)
            scale = cap / current_max
            df.loc[mask, 'WIND'] = (storm_winds * scale).clip(lower=22.0)
            
            # Recalculate pressure
            df.loc[mask, 'PRES'] = calculate_pressure_from_wind(
                df.loc[mask, 'WIND'].values,
                env_pressure=1010.0,
                lat=df.loc[mask, 'LAT'].values
            )
            # Recalculate category labels
            new_cats = []
            for w in df.loc[mask, 'WIND']:
                if w >= 100: new_cats.append("Super Typhoon")
                elif w >= 64: new_cats.append("Typhoon")
                elif w >= 48: new_cats.append("Severe Tropical Storm")
                elif w >= 34: new_cats.append("Tropical Storm")
                elif w >= 22: new_cats.append("Tropical Depression")
                else: new_cats.append("Remnant Low")
            df.loc[mask, 'CATEGORY'] = new_cats
            
            clamped_count += 1
            logging.info(f"SCS FINAL CLAMP: {sid} max {current_max:.1f} -> "
                         f"{df.loc[mask, 'WIND'].max():.1f} kt (cap={cap:.0f})")
    
    logging.info(f"SCS FINAL CLAMP: Adjusted {clamped_count}/{len(scs_sids)} "
                 f"SCS-genesis storms")
    return df
...
def generate_yearly_storms(year, total_storms=None, par_only=True):
    ...
    for month, count in month_counts.items():
        ...
        while len(month_storms) < count and actual_attempts < max_attempts:
            ...
            try:
                ...
                storm = safe_storm_generation(year, month, used_ids, max_attempts=10)
                if storm is None:
                ...
                if par_only:
                    ...
                    # UNLOCKED: Keep the storm even if it doesn't pass PAR
                    if True: 
                        month_storms.append(storm)
                        used_ids.add(storm['SID'].iloc[0])
                        consecutive_failures = 0  # Reset on success
        ...
        if len(month_storms) < count:
            ...
            # FIX-A: Emergency generation with delta-shift (no first-point-only overwrite)
            emergency_attempts = 0
            while len(month_storms) < count and emergency_attempts < shortfall * 20:
                emergency_attempts += 1
                try:
                    # Pick a safe genesis location
                    for _attempt in range(30):
                        if month in [7, 8, 9, 10]:  # Peak season
                        ...
                        if not is_over_land(lon, lat) and not is_in_genesis_exclusion_zone(lon, lat):
                            break

                    # Generate storm normally
                    storm = generate_synthetic_storm(year, month, _df_positions, _historical_wind_speeds, used_ids)

                    if storm is not None and not storm.empty:
                        # FIX-A: Delta-shift the ENTIRE track to the safe genesis
                        orig_lat0 = storm.iloc[0]['LAT']
                        orig_lon0 = storm.iloc[0]['LON']
                        dlat = lat - orig_lat0
                        dlon = lon - orig_lon0
                        storm['LAT'] = storm['LAT'] + dlat
                        storm['LON'] = storm['LON'] + dlon

                        # Re-check PAR entry after translation
                        ...
                        #    used_ids.add(storm['SID'].iloc[0])
                        #    logging.info(f"Emergency generation successful (delta-shifted), now have {len(month_storms)}/{count}")
                        
                        # Re-check PAR entry after translation (Kept just for your information)
                        passes_par = any(is_inside_par(r['LAT'], r['LON'], use_buffer=False) 
                                       for _, r in storm.iterrows())
                        
                        # UNLOCKED: Accept the storm even if passes_par is False
                        if True:
                            month_storms.append(storm)
                            storm_dfs.append(storm)
                            used_ids.add(storm['SID'].iloc[0])
                            
                            # I added '{passes_par}' here so your logs will tell you if it was an inside or outside storm!
                            logging.info(f"Emergency generation successful (delta-shifted), now have {len(month_storms)}/{count}. Entered PAR: {passes_par}")
    ...
    if storm_dfs:
        ...
        # UNLOCKED: Set min_par_points to 0 so it doesn't delete external storms
        filtered_storms = filter_storms_for_realism(yearly_storms, min_par_points=0)
        ...
        if not filtered_storms.empty:
            ...
            max_attempts = 100  # Generous retry budget
            attempts = 0
            stall_count = 0     # Track consecutive failed batches

            while actual_count < total_storms and attempts < max_attempts:
                attempts += 1
                ...
                if additional_storms:
                    ...
                    stall_count += 1
                    if stall_count >= 10:
                        logging.warning(f"Gave up after {stall_count} consecutive failed batches for year {year}")
                        break

            if actual_count < total_storms:
                shortfall = total_storms - actual_count
                logging.warning(f"Year {year}: generated {actual_count}/{total_storms} "
                                f"(shortfall={shortfall}) after {attempts} backfill attempts")
...
def generate_multi_year_storms(start_year, end_year, output_directory, target_storm_count=None):
    ...
    if target_storm_count is not None and all_storms:
                ...
                for year in years_to_add:
                    ...
                    while attempts < 30:
                        try:
                            ...
                            #    used_ids.add(storm['SID'].iloc[0])
                            #    break
                            # This will save the storm regardless of whether it enters the PAR or not
                            if True:
                                additional_storms.append(storm)
                                used_ids.add(storm['SID'].iloc[0])
                                break
        ...
        # Hard-check: report exact shortfall/surplus vs forecast
        if forecast_total > 0:
            diff = total_storm_count - forecast_total
            if abs(diff) <= 5:
                logging.info(f"STORM COUNT CHECK: {total_storm_count}/{forecast_total} "
                             f"(diff={diff:+d}, within ±5 tolerance)")
            else:
                logging.warning(f"STORM COUNT CHECK: {total_storm_count}/{forecast_total} "
                                f"(diff={diff:+d}, OUTSIDE ±5 tolerance)")
...
def generate_additional_par_storms(count, start_year, end_year, existing_ids=None):
    ...
    while len(additional_storms) < count and attempts < max_attempts:
        ...
        try:
            # Generate a storm
            storm = safe_storm_generation(year, month, existing_ids, max_attempts=10)
            if storm is None:
                continue
...
def plot_storm_tracks(storms_df, output_path=None, show_plot=True):
    ...
    # Set up map projection — leave left margin for legend
    ax = fig.add_axes([0.08, 0.10, 0.74, 0.85], projection=ccrs.PlateCarree())
    ...
    # Create custom legend — inside map, lower-left (over ocean)
    ...
    leg = ax.legend(handles=legend_elements, loc='upper left',
               ...
               frameon=True, facecolor='white', framealpha=0.95, edgecolor='gray',
...
def enforce_strict_geography_caps(df):
    """
    Absolute last line of defense against extreme winds over land or recovering 
    to STY after crossing land. Applied AFTER wind scaling to enforce constraints.
    """
    modified_df = df.copy()
    
    # Process per-storm to track post-landfall status
    for sid in modified_df['SID'].unique():
        storm_mask = modified_df['SID'] == sid
        storm_indices = modified_df[storm_mask].index
        
        crossed_land = False
        for i in storm_indices:
            lon = modified_df.loc[i, 'LON']
            lat = modified_df.loc[i, 'LAT']
            wind = modified_df.loc[i, 'WIND']
            
            is_land = is_over_land(lon, lat)
            if is_land:
                crossed_land = True
                
                # ARCHIPELAGO FILTER
                if wind >= 96:
                    modified_df.loc[i, 'WIND'] = 85.0
                elif wind >= 64:
                    elev = get_elevation(lon, lat)
                    if elev > 300:
                        modified_df.loc[i, 'WIND'] = min(wind, 65.0)
            else:
                # POST-LANDFALL OVER-WATER CAP
                if crossed_land and wind >= 100:
                    modified_df.loc[i, 'WIND'] = 95.0
            
            # WESTERN CAPS
            current_wind = modified_df.loc[i, 'WIND']
            # West of Luzon mountains (lon < 120.5)
            if lon < 120.5 and 14.0 <= lat <= 19.0 and current_wind >= 100:
                modified_df.loc[i, 'WIND'] = min(current_wind, 90.0)
            # West of Visayas (lon < 123)
            elif lon < 123.0 and 9.5 <= lat <= 12.5 and current_wind >= 100:
                modified_df.loc[i, 'WIND'] = min(current_wind, 90.0)
            # West of Mindanao (lon < 124)
            elif lon < 124.0 and 6.5 <= lat <= 10.0 and current_wind >= 100:
                modified_df.loc[i, 'WIND'] = min(current_wind, 90.0)
            # General: nothing >= 106kt west of 120 over Philippine latitudes
            elif lon < 120.0 and 5.0 <= lat <= 21.0 and current_wind >= 106:
                modified_df.loc[i, 'WIND'] = 95.0

    # Recompute categories
    cats = []
    for w in modified_df['WIND']:
        if w >= 100: cats.append("Super Typhoon")
        elif w >= 64: cats.append("Typhoon")
        elif w >= 48: cats.append("Severe Tropical Storm")
        elif w >= 34: cats.append("Tropical Storm")
        elif w >= 22: cats.append("Tropical Depression")
        else: cats.append("Remnant Low")
    modified_df['CATEGORY'] = cats
    
    # Recalculate pressures based on the newly capped winds
    modified_df['PRES'] = calculate_pressure_from_wind(
        modified_df['WIND'].values, 
        env_pressure=1010.0, 
        lat=modified_df['LAT'].values
    )
    
    return modified_df
    
def scale_and_cap_to_exact_mean(df, target_mean=58.0, tolerance=0.05, max_iters=15):
    """
    Iteratively scales winds and applies geographic caps until the ensemble 
    mean exactly matches the target, redistributing capped energy to valid points.
    """
    modified_df = df.copy()
    
    for iteration in range(max_iters):
        current_mean = modified_df['WIND'].mean()
        
        # Break if we've hit the target mean within the tolerance
        if abs(current_mean - target_mean) <= tolerance:
            break
            
        # Apply scaling factor — but protect >=100 kt winds from upward scaling
        # (same logic as scale_winds_to_target_mean; prevents pushing 110-118 kt
        # points into the 120-126 range when scaling_factor > 1)
        scaling_factor = target_mean / current_mean
        extreme_mask = modified_df['WIND'] >= 100.0
        extreme_saved = modified_df.loc[extreme_mask, 'WIND'].copy()
        
        modified_df['WIND'] = modified_df['WIND'] * scaling_factor
        
        # Restore extreme winds that were pushed upward by scaling
        if extreme_mask.any() and scaling_factor > 1.0:
            modified_df.loc[extreme_mask, 'WIND'] = extreme_saved
        
        # Immediately re-apply geographic caps to cut down any points 
        # that the scaling factor pushed past the physical limits
        modified_df = enforce_strict_geography_caps(modified_df)
        
    logging.info(f"Converged on mean {modified_df['WIND'].mean():.2f} kt after {iteration+1} iterations.")
    return modified_df
...
def generate_storm_ensemble(start_year, end_year, ensemble_size=50, base_seed=42, target_storms=1824):
    ...
    for ensemble_id in range(1, ensemble_size+1):
        ...
        #member_seed = base_seed + ensemble_id * 1000 # for uniformity
        member_seed = int(time.time() * 1000) % (2**31) + ensemble_id * 1000 #for randomness 
        ...
        if all_storms:
            ...
            # FIX-C: Wind scaling moved to AFTER storm-count adjustment (see below).
            # The early call here was being diluted by unscaled patching storms.
            ...
            # ——— FIX-C: Scale winds AFTER all storm-count adjustments are complete ———
            # This ensures patching storms are also scaled to the 58 kt target.
            #combined_storms = scale_winds_to_target_mean(combined_storms, target_mean=58.0)
        
            # CRITICAL FIX: Re-apply strict geographic caps AFTER scaling.
            # The scaling multiplier otherwise pushes attenuated storms back up to extreme levels.
            #logging.info(f"Ensemble {ensemble_id}: Applying strict geography caps post-scaling...")
            #combined_storms = enforce_strict_geography_caps(combined_storms)
        
            #combined_storms = compensate_for_land_losses(combined_storms, target_sty_percent=5.49)
            
            # ——— FIX-C: Iteratively scale and cap to guarantee exactly 58.0 kt ———
            logging.info(f"Ensemble {ensemble_id}: Iteratively scaling to exactly 58.0 kt while enforcing caps...")
            combined_storms = scale_and_cap_to_exact_mean(combined_storms, target_mean=58.0)

            # ——— STY FRACTION ENFORCEMENT (runs AFTER scaling to avoid re-inflation) ———
            # Filtering enriches STY storms (long tracks survive, short TDs don't).
            # Correct by downgrading excess STY storms to strong TY (scale peak to 95 kt).
            _target_sty_point_pct = 5.5
            _current_sty_points = (combined_storms['WIND'] >= 100).sum()
            _total_points = len(combined_storms)
            _current_sty_pct = 100 * _current_sty_points / _total_points if _total_points > 0 else 0
            
            if _current_sty_pct > _target_sty_point_pct:
                _sty_storms = []
                for _sid in combined_storms['SID'].unique():
                    _smask = combined_storms['SID'] == _sid
                    _smax = combined_storms.loc[_smask, 'WIND'].max()
                    if _smax >= 100:
                        _sty_storms.append((_sid, _smax))
                _sty_storms.sort(key=lambda x: x[1])  # weakest STY first
                
                _downgraded = 0
                for _sid, _smax in _sty_storms:
                    _smask = combined_storms['SID'] == _sid
                    _scale = 95.0 / _smax
                    combined_storms.loc[_smask, 'WIND'] = (
                        combined_storms.loc[_smask, 'WIND'] * _scale
                    ).clip(lower=22.0)
                    _downgraded += 1
                    
                    _new_sty_pts = (combined_storms['WIND'] >= 100).sum()
                    _new_pct = 100 * _new_sty_pts / _total_points
                    if _new_pct <= _target_sty_point_pct:
                        break
                
                _cats = []
                for _w in combined_storms['WIND']:
                    if _w >= 100: _cats.append("Super Typhoon")
                    elif _w >= 64: _cats.append("Typhoon")
                    elif _w >= 48: _cats.append("Severe Tropical Storm")
                    elif _w >= 34: _cats.append("Tropical Storm")
                    elif _w >= 22: _cats.append("Tropical Depression")
                    else: _cats.append("Remnant Low")
                combined_storms['CATEGORY'] = _cats
                combined_storms['PRES'] = calculate_pressure_from_wind(
                    combined_storms['WIND'].values, env_pressure=1010.0,
                    lat=combined_storms['LAT'].values
                )
                logging.info(f"STY ENFORCEMENT: Downgraded {_downgraded} STY storms to TY. "
                             f"STY points: {_current_sty_pct:.1f}% -> {_new_pct:.1f}%")

            # NOTE: compensate_for_land_losses DISABLED — the open-water STY
            # continuity fixes (hysteresis, gap-fill, peak floor) now preserve
            # STY intensity over water naturally. This function was artificially
            # promoting TY→STY and STY→124+ kt, causing the purple histogram spike.
            # combined_storms = compensate_for_land_losses(combined_storms, target_sty_percent=5.49)
            
            # Final wind validation
            ...
            # ——— v7p1: SCS GENESIS final-pass intensity clamp (MUST be last) ———
            logging.info(f"Ensemble {ensemble_id}: Applying final SCS genesis intensity clamp...")
            combined_storms = clamp_scs_genesis_intensity(combined_storms)
            ...
            # ——— SCS / Sulu Sea genesis diagnostics ———
            try:
                diagnose_scs_sulu_genesis(combined_storms, start_year, end_year, output_dir=ensemble_dir)
            except Exception as e:
                logging.error(f"SCS/Sulu diagnostics failed for ensemble {ensemble_id}: {e}")
...
# ——— SCS / Sulu Sea Genesis Diagnostics ———
def diagnose_scs_sulu_genesis(storms_df, start_year=None, end_year=None, output_dir=None):
    """
    Count synthetic genesis events inside the SCS and Sulu Sea boxes
    (matching the IBTrACS definition in SCS_SULU_genesis_by_year.csv)
    and compare against historical climatology.
    
    Boxes:
        SCS:  5–25°N, 105–120°E
        Sulu: 4–13°N, 118–123°E
    """
    if storms_df is None or storms_df.empty:
        logging.warning("diagnose_scs_sulu_genesis: empty input")
        return {}
    ...
    for sid in storms_df['SID'].unique():
        storm = storms_df[storms_df['SID'] == sid]
        gen = storm.iloc[0]
        g_lat, g_lon = gen['LAT'], gen['LON']
        g_wind = ensure_scalar(gen['WIND'], 25.0)
        
        if 5.0 <= g_lat <= 25.0 and 105.0 <= g_lon <= 120.0:
            scs_sids.add(sid)
        if 4.0 <= g_lat <= 13.0 and 118.0 <= g_lon <= 123.0:
            sulu_sids.add(sid)
            sulu_detail.append((sid, g_lat, g_lon, g_wind))
    
    total_storms = storms_df['SID'].nunique()
    n_years = (end_year - start_year + 1) if (start_year and end_year) else 101
    
    scs_count = len(scs_sids)
    sulu_count = len(sulu_sids)
    scs_per_yr = scs_count / n_years
    sulu_per_yr = sulu_count / n_years
    
    logging.info("=" * 70)
    logging.info("SCS / SULU SEA GENESIS DIAGNOSTICS")
    logging.info("=" * 70)
    logging.info(f"Total synthetic storms: {total_storms}  |  Simulation years: {n_years}")
    logging.info(f"SCS genesis:  {scs_count} storms  ({scs_per_yr:.2f}/yr)  "
                 f"[historical: {_HIST_SCS_MEAN_PER_YEAR:.2f}/yr]")
    logging.info(f"Sulu genesis: {sulu_count} storms  ({sulu_per_yr:.2f}/yr)  "
                 f"[historical: {_HIST_SULU_MEAN_PER_YEAR:.2f}/yr]")
    
    if sulu_detail:
        logging.info(f"Sulu Sea genesis detail ({len(sulu_detail)} events):")
        for sid, lat, lon, wind in sulu_detail:
            cat = "RL" if wind < 22 else ("TD" if wind < 34 else ("TS" if wind < 64 else "TY+"))
            logging.info(f"  {sid}: ({lat:.2f}N, {lon:.2f}E)  {wind:.0f}kt  [{cat}]")
        sulu_winds = [w for _, _, _, w in sulu_detail]
        ts_or_above = sum(1 for w in sulu_winds if w >= 34)
        if ts_or_above > 0:
            logging.warning(f"  WARNING: {ts_or_above} Sulu genesis events at TS+ intensity!")
    
    scs_dev = (scs_per_yr - _HIST_SCS_MEAN_PER_YEAR) / _HIST_SCS_MEAN_PER_YEAR * 100
    sulu_dev = ((sulu_per_yr - _HIST_SULU_MEAN_PER_YEAR) / _HIST_SULU_MEAN_PER_YEAR * 100
                if _HIST_SULU_MEAN_PER_YEAR > 0 else 0)
    logging.info(f"Deviation from historical — SCS: {scs_dev:+.1f}%  |  Sulu: {sulu_dev:+.1f}%")
    logging.info("=" * 70)
    
    # Per-year breakdown
    yearly_scs = {}
    yearly_sulu = {}
    if 'YEAR' in storms_df.columns and start_year and end_year:
        for yr in range(start_year, end_year + 1):
            yearly_scs[yr] = 0
            yearly_sulu[yr] = 0
        for sid in scs_sids:
            yr = storms_df[storms_df['SID'] == sid].iloc[0].get('YEAR', None)
            if yr is not None:
                yearly_scs[int(yr)] = yearly_scs.get(int(yr), 0) + 1
        for sid in sulu_sids:
            yr = storms_df[storms_df['SID'] == sid].iloc[0].get('YEAR', None)
            if yr is not None:
                yearly_sulu[int(yr)] = yearly_sulu.get(int(yr), 0) + 1
        
        sulu_nonzero = sum(1 for v in yearly_sulu.values() if v > 0)
        logging.info(f"Sulu nonzero years: {sulu_nonzero}/{n_years} ({100*sulu_nonzero/n_years:.1f}%)  "
                     f"[historical: {100*_HIST_SULU_NONZERO_FRAC:.1f}%]")
    
    # Write CSV for comparison
    if output_dir and yearly_scs and yearly_sulu:
        diag_path = os.path.join(output_dir, "synthetic_SCS_SULU_genesis_by_year.csv")
        try:
            with open(diag_path, 'w') as f:
                f.write("year,SCS_count,SULU_count\n")
                for yr in sorted(yearly_scs.keys()):
                    f.write(f"{yr},{yearly_scs.get(yr,0)},{yearly_sulu.get(yr,0)}\n")
            logging.info(f"Wrote SCS/Sulu diagnostics to {diag_path}")
        except Exception as e:
            logging.error(f"Failed to write SCS/Sulu diagnostic CSV: {e}")
    
    return {
        'scs_total': scs_count, 'sulu_total': sulu_count,
        'scs_per_yr': scs_per_yr, 'sulu_per_yr': sulu_per_yr,
    }
...
def main():
   ...
   # Initialize the DEM for storm calculations
    ...
    start_year = 2026
    end_year = 2125
    target_storms = 1879  # Exact number of storms to generate
    ...
    if not ensemble_mode:
        ...
        if all_storms:
            ...
            # Apply SINGLE final wind scaling to achieve exact target mean
            combined_storms = scale_winds_to_target_mean(combined_storms, target_mean=58.0)

            # ——— v7p1: SCS GENESIS final-pass intensity clamp (MUST be last) ———
            logging.info("Applying final SCS genesis intensity clamp...")
            combined_storms = clamp_scs_genesis_intensity(combined_storms)
            ...
            # ——— SCS / Sulu Sea genesis diagnostics ———
            try:
                diagnose_scs_sulu_genesis(combined_storms, start_year, end_year, output_dir=run_dir)
            except Exception as e:
                logging.error(f"SCS/Sulu diagnostics failed: {e}")
